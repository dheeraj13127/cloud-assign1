...
def predict(model, src_img, dst_img,api_type):
    ...
    if api_type=="annotated":
        ...
        return results[0].boxes,result[0].keypoints,len(keypoints.xy),result[0].speed
...
def main():
    try:
        ...
        output_image = 'bus_with_keypoints.jpg'
        boxes,keypoints,count,speed = predict(model,image_path,output_image)
        log.info(boxes,keypoints,count,speed)
